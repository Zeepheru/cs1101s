...
function evaluate(program) { 
    ...
    while (! is_null(C)) {
        ...
        if (is_literal(command)) {
            ...
            const body = reorder_sequence(sequence_statements(command));
...
function reorder_sequence(seq) {
    ...
    // apparently u need to insert pop lol
...
function scan_out_names(component) {
    display(component);
    display(head(component));
    return is_complex_function(component) || is_block(component)
            ? null
            : is_name(component) 
            ? list(symbol_of_name(component))
            : is_pair(head(component))
            ? map(scan_out_names, component)
            : accumulate(append,
                        null,
                        map(scan_out_names,
                            sequence_statements(component)));
